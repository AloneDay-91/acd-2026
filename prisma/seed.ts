...
const hotels = [
  { name: "Hôtel de la Poste", address: "10 place de la Libération", postalCode: "10000", city: "Troyes", phone: "03 25 73 05 05", email: "contact@hoteldelaposte.fr", websiteUrl: "https://www.hoteldelaposte.fr", latitude: 48.2973, longitude: 4.0744 },
  { name: "Ibis Troyes Centre", address: "22 rue Général de Gaulle", postalCode: "10000", city: "Troyes", phone: "03 25 71 11 11", email: "contact@ibistroyes.com", websiteUrl: "https://www.ibis.com", latitude: 48.2985, longitude: 4.0710 },
  { name: "Best Western La Chaumière", address: "Route de Sens", postalCode: "10600", city: "La Rivière-de-Corps", phone: "03 25 75 02 31", email: "lachaumiere@bestwestern.fr", websiteUrl: "https://www.bestwestern.fr", latitude: 48.2941, longitude: 4.0352 },
  { name: "Mercure Troyes Centre", address: "11 rue des Bas Trevois", postalCode: "10000", city: "Troyes", phone: "03 25 46 28 28", email: "H2083@accor.com", websiteUrl: "https://all.accor.com/mercure", latitude: 48.2951, longitude: 4.0768 },
  { name: "Kyriad Troyes Centre", address: "44 avenue Chomedey de Maisonneuve", postalCode: "10000", city: "Troyes", phone: "03 25 73 11 88", email: "troyes.centre@kyriad.fr", websiteUrl: "https://troyes-centre.kyriad.com", latitude: 48.2965, longitude: 4.0712 },
  { name: "B&B HOTEL Troyes Centre", address: "11 Boulevard du 14 Juillet", postalCode: "10000", city: "Troyes", phone: "08 92 78 81 05", email: "bb_4618@hotelbb.com", websiteUrl: "https://www.hotel-bb.com", latitude: 48.2938, longitude: 4.0701 },
];
...
// ============================================
async function main() {
  console.log(DEMO_MODE ? "🎬 Mode DÉMO activé" : "🚀 Mode normal");
  ...
  const createdIuts = await Promise.all(iuts.map((iut) => prisma.iut.create({ data: iut })));
  ...
  console.log(`🍽️  Création de ${meals.length} repas avec options...`);
  ...
  await prisma.setting.upsert({
    ...
    create: {
      headerBadgeText: DEMO_MODE ? "DÉMO" : "Édition 2026",
      siteName: DEMO_MODE ? "ACD MMI 2026 — Démo" : "ACD MMI 2026",
  ...
  await prisma.homeContent.upsert({ where: { id: "home_content" }, create: {}, update: {} });
  await prisma.accessContent.upsert({ where: { id: "access_content" }, create: {}, update: {} });
  await prisma.programmeContent.upsert({ where: { id: "programme_content" }, create: {}, update: {} });
  await prisma.inscriptionContent.upsert({ where: { id: "inscription_content" }, create: {}, update: {} });
  await prisma.hotelsContent.upsert({ where: { id: "hotels_content" }, create: {}, update: {} });

  const TOTAL_REGISTRATIONS = 45;
  console.log(`👥 Création de ${TOTAL_REGISTRATIONS} inscriptions...`);
  ...
  for (let i = 0; i < TOTAL_REGISTRATIONS; i++) {
    ...
    const allergens = chance(0.15) ? randomItem(["Arachide", "Lactose", "Gluten", "Fruits à coque"]) : null;
    ...
    const selectedMeals = [...createdMeals].sort(() => Math.random() - 0.5).slice(0, mealCount);

    const activityCount = Math.floor(Math.random() * 3);
    const selectedActivities = [...createdActivities].sort(() => Math.random() - 0.5).slice(0, activityCount);

    let totalPrice = selectedMeals.length * 12.50;
    for (const a of selectedActivities) totalPrice += Number(a.price);
    ...
    const registration = await prisma.registration.create({
      data: {
        ...
        iutId: iut.id,
        allergens,
        isMotorized, isVegetarian, isVegan, noPork, noAlcohol,
        totalPrice,
        status,
    ...
    for (const a of selectedActivities) {
      await prisma.registrationActivity.create({
        data: { registrationId: registration.id, activityId: a.id },
      });
    }
